    ...
    if ($action === 'fresh_db' || $action === 'import_sql') {
        ...
        if ($confirm !== 'RESET_CONFIRMED') {
            ...
            try {
                ...
                // Guarantee zero demo products, zero demo categories, zero demo brands
                try {
                    $pdo->exec("SET FOREIGN_KEY_CHECKS=0;");
                    $cleanTables = ['order_items', 'orders', 'reseller_listings', 'product_images', 'products', 'categories', 'brands'];
                    foreach ($cleanTables as $ct) {
                        try {
                            $pdo->exec("TRUNCATE TABLE `{$ct}`;");
                        } catch (\Throwable $ex) {}
                    }
                    $pdo->exec("SET FOREIGN_KEY_CHECKS=1;");
                } catch (\Throwable $ex) {}
                ...
                $catCount = 0;
                if (in_array('categories', $vTables)) {
                    $catCount = (int)$pdo->query("SELECT COUNT(*) FROM `categories`")->fetchColumn();
                }
                $brandCount = 0;
                if (in_array('brands', $vTables)) {
                ...
                out(">>> Verified Categories: {$catCount} (100% Clean!)");
                out(">>> Verified Brands: {$brandCount} (100% Clean!)");
