...
function PoliciesAdmin() {
  const [policies, setPolicies] = useState<ResellerPolicy[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [formData, setFormData] = useState<PolicyFormState>(EMPTY_FORM);
  ...
  const openEditModal = (p: ResellerPolicy) => {
    setFormData({
      id: p.id,
      title: p.title,
      summary: p.summary || "",
  ...
  const toggleBulkMode = () => {
    if (!bulkMode) {
      setBulkText(pointsToText(formData.points.filter((p) => p.trim() !== "")));
    } else {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const cleanPoints = bulkMode
      ? pointsFromText(bulkText)
      : formData.points.map((p) => p.trim()).filter(Boolean);
  ...
  async function toggleActive(p: ResellerPolicy) {
    try {
      const { error } = await supabase
        .from("reseller_policies")
  ...
  async function handleDelete(p: ResellerPolicy) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {/* Quick Stats & Starters Banner */}
      <div className="surface-card p-5 rounded-2xl border border-border/80 shadow-2xs flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary">
            <ScrollText className="h-5 w-5" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-foreground">মোট পলিসি সেকশন: {policies.length}টি</h3>
            <p className="text-xs text-muted-foreground">সক্রিয় রয়েছে: {policies.filter((p) => p.is_active).length}টি</p>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs font-medium text-muted-foreground">রেডিমেড টেমপ্লেট:</span>
      ...
      {/* Policies Grid */}
      {policies.length === 0 ? (
        <div className="surface-card p-12 text-center rounded-2xl border border-dashed border-border/80">
          <div className="grid h-14 w-14 place-items-center rounded-2xl bg-primary/10 text-primary mx-auto mb-3">
            <ScrollText className="h-7 w-7" />
          </div>
          <h3 className="text-base font-bold text-foreground">কোনো পলিসি যুক্ত করা হয়নি</h3>
          <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
            রিসেলারদের জন্য ডেলিভারি, কমিশন বা রিটার্ন সংক্রান্ত নীতিমালা তৈরি করতে উপরের "Add New Policy" বাটনে ক্লিক করুন।
          </p>
          <button
            type="button"
            onClick={openNewModal}
          ...
          >
            <Plus className="h-4 w-4" /> প্রথম পলিসি যুক্ত করুন
          </button>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {policies.map((p, index) => (
            <div
              key={p.id}
              className={cn(
                "surface-card relative flex flex-col justify-between rounded-2xl border p-5 shadow-2xs transition-all",
                p.is_active
              ...
              <div>
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-start gap-3">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary font-bold text-sm">
                      {p.sort_order || index + 1}
                    </span>
                    <div>
                      <h3 className="text-base font-bold text-foreground leading-snug">{p.title}</h3>
                      {p.summary && <p className="text-xs text-muted-foreground mt-0.5">{p.summary}</p>}
                    </div>
                  </div>

                  <span
                    className={cn(
                      "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold shrink-0",
                      p.is_active
                ...
                <ul className="space-y-2 my-4 pl-1">
                  {p.points.map((pt, ptIdx) => (
                    <li key={ptIdx} className="flex items-start gap-2 text-xs sm:text-sm text-muted-foreground leading-relaxed">
      ...
      {modalOpen && (
        ...
        >
          <form onSubmit={handleSave} className="space-y-4">
            ...
            {!formData.id && (
              <div className="rounded-xl border border-primary/20 bg-primary/5 p-3">
                <div className="text-[11px] font-bold text-primary mb-2 flex items-center gap-1.5">
                  ...
                  <span>দ্রুত রেডিমেড টেমপ্লেট ব্যবহার করুন:</span>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              <div className="sm:col-span-3">
                <label className="mb-1 block text-xs font-bold text-foreground">
                  পলিসির নাম / শিরোনাম <span className="text-destructive">*</span>
                </label>
                <input
                  required
            ...
            <div>
              <label className="mb-1 block text-xs font-bold text-foreground">
                এক লাইনে বিবরণ / সাব-টাইটেল (Optional)
              </label>
              <input
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold text-foreground flex items-center gap-1.5">
                  ...
                  <span>পলিসির পয়েন্ট ও নিয়মাবলী</span>
                </label>
                <button
                  type="button"
                ...
                >
                  {bulkMode ? "Switch to Point-by-Point" : "Switch to Bulk Text Mode"}
                </button>
              </div>
              ...
              {bulkMode ? (
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    প্রতিটি নতুন লাইনে একটি করে বুলেট পয়েন্ট তৈরি হবে।
                  </p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1 no-scrollbar">
                  ...
                  >
                    ...
                    <span>নতুন পয়েন্ট যোগ করুন (+ Add Point)</span>
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-border/70 bg-muted/20 p-3">
              <div>
                <div className="text-xs font-bold text-foreground">রিসেলারদের কাছে দৃশ্যমান (Active Status)</div>
                <div className="text-[11px] text-muted-foreground">সক্রিয় রাখলে রিসেলাররা তাদের ড্যাশবোর্ডে দেখতে পারবে।</div>
              </div>
              <button
                type="button"
              ...
              >
                {formData.is_active ? "Published" : "Hidden"}
              </button>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-border/60">
              ...
              >
                ...
                <span>{formData.id ? "Update Policy" : "Create Policy"}</span>
              </button>
            </div>
          </form>
        </AppModal>
