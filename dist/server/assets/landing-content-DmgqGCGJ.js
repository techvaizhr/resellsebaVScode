...
//#endregion
//#region src/lib/landing-content.ts
var DEFAULT_LANDING_CONTENT = {
	...
	footer: { tagline: "Empowering next-generation online entrepreneurs across Bangladesh." }
};
...
function mergeLandingContent(raw) {
	...
	if (!saved || typeof saved !== "object") return JSON.parse(JSON.stringify(DEFAULT_LANDING_CONTENT));
	...
	return {
		...
		hero: {
			...
			badges: Array.isArray(saved.hero?.badges) && saved.hero.badges.length > 0 ? saved.hero.badges : def.hero.badges,
			bannerImage: saved.hero?.bannerImage ?? def.hero.bannerImage
		},
		stats: {
			title: saved.stats?.title ?? def.stats?.title,
			items: Array.isArray(saved.stats?.items) && saved.stats.items.length > 0 ? saved.stats.items : def.stats?.items ?? []
		},
		about: {
			badge: saved.about?.badge ?? def.about?.badge ?? "",
			title: saved.about?.title ?? def.about?.title ?? "",
			body: saved.about?.body ?? def.about?.body ?? "",
			points: Array.isArray(saved.about?.points) && saved.about.points.length > 0 ? saved.about.points : def.about?.points ?? [],
			flow: Array.isArray(saved.about?.flow) && saved.about.flow.length > 0 ? saved.about.flow : def.about?.flow ?? []
		},
		features: {
			title: saved.features?.title ?? def.features.title,
			subtitle: saved.features?.subtitle ?? def.features.subtitle,
			items: Array.isArray(saved.features?.items) && saved.features.items.length > 0 ? saved.features.items : def.features.items
		},
		how: {
			title: saved.how?.title ?? def.how.title,
			subtitle: saved.how?.subtitle ?? def.how.subtitle,
			steps: Array.isArray(saved.how?.steps) && saved.how.steps.length > 0 ? saved.how.steps : def.how.steps
		},
		faq: {
			title: saved.faq?.title ?? def.faq?.title ?? "",
			subtitle: saved.faq?.subtitle ?? def.faq?.subtitle ?? "",
			items: Array.isArray(saved.faq?.items) && saved.faq.items.length > 0 ? saved.faq.items : def.faq?.items ?? []
		},
		cta: {
		...
		footer: { tagline: saved.footer?.tagline ?? def.footer.tagline }
	};
}
//#endregion
export { APP_ICON_NAMES as i, mergeLandingContent as n, APP_ICONS as r, DEFAULT_LANDING_CONTENT as t };
