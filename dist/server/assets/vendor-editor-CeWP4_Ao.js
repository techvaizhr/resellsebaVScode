...
import { c as deepEqual, m as require_react } from "./vendor-charts-kQ5QBPqu.js";
...
position).
*/
var TextSelection = class TextSelection extends Selection$1 {
	/**
	...
	*/
	...
	map(doc, mapping) {
		...
		if (!$head.parent.inlineContent) return Selection$1.near($head);
	...
	*/
	static between($anchor, $head, bias) {
		...
		if (!$head.parent.inlineContent) {
			...
			else return Selection$1.near($head, bias);
		}
...
*/
var NodeSelection = class NodeSelection extends Selection$1 {
	/**
	...
	*/
	...
	map(doc, mapping) {
		...
		let $pos = doc.resolve(pos);
		if (deleted) return Selection$1.near($pos);
...
document).
*/
var AllSelection = class AllSelection extends Selection$1 {
	/**
	...
	*/
	insertText(text, from, to) {
		...
		if (from == null) {
			...
			if (!this.selection.empty && this.selection.to == from + text.length) this.setSelection(Selection$1.near(this.selection.$to));
			return this;
		}
	}
	/**
	...
	*/
	static fromJSON(config, json, pluginFields) {
		...
		$config.fields.forEach((field) => {
			...
			else if (field.name == "selection") instance.selection = Selection$1.fromJSON(instance.doc, json.selection);
	...
	if ($cursor.parent.content.size == 0 && (textblockAt(before, "end") || NodeSelection.isSelectable(before))) for (let depth = $cursor.depth;; depth--) {
		...
		if (delStep && delStep.slice.size < delStep.to - delStep.from) {
			if (dispatch) {
				...
				tr.setSelection(textblockAt(before, "end") ? Selection$1.findFrom(tr.doc.resolve(tr.mapping.map($cut.pos, -1)), -1) : NodeSelection.create(tr.doc, $cut.pos - before.nodeSize));
...
var joinForward = (state, dispatch, view) => {
	...
	if ($cursor.parent.content.size == 0 && (textblockAt(after, "start") || NodeSelection.isSelectable(after))) {
		...
		if (delStep && delStep.slice.size < delStep.to - delStep.from) {
			if (dispatch) {
				let tr = state.tr.step(delStep);
				tr.setSelection(textblockAt(after, "start") ? Selection$1.findFrom(tr.doc.resolve(tr.mapping.map($cut.pos)), 1) : NodeSelection.create(tr.doc, tr.mapping.map($cut.pos)));
				dispatch(tr.scrollIntoView());
			}
			return true;
		}
...
var DOMObserver = class {
	...
	flush() {
		...
		let readSel = null;
		if (from < 0 && newSel && view.input.lastFocus > Date.now() - 200 && Math.max(view.input.lastTouch, view.input.lastClick.time) < Date.now() - 300 && selectionCollapsed(sel) && (readSel = selectionFromDOM(view)) && readSel.eq(Selection$1.near(view.state.doc.resolve(0), 1))) {
...
var handleTab = (editor, name, parentListTypes) => {
	...
	if (!lastItem.canReplace(lastItem.childCount, lastItem.childCount, Fragment$1.from(block))) return false;
	const blockStart = $from.before();
	const blockEnd = $from.after();
	const insideLastItemEnd = blockStart - 2;
	return editor.commands.command(({ tr, dispatch }) => {
		if (dispatch) {
			tr.delete(blockStart, blockEnd).insert(insideLastItemEnd, Fragment$1.from(block));
...
var TaskItem = Node.create({
	...
	renderMarkdown: (node, h) => {
		var _node$attrs;
		return renderNestedMarkdownContent(node, h, `- [${((_node$attrs = node.attrs) === null || _node$attrs === void 0 ? void 0 : _node$attrs.checked) ? "x" : " "}] `);
	},
	addExtensions() {
...
var src_default$1 = Extension.create({
	name: "starterKit",
	addExtensions() {
		const extensions = [];
	...
	addNodeView() {
		...
		return ({ node, getPos, HTMLAttributes, editor }) => {
			...
			const onUpdate = (updatedNode) => {
				...
				const newHTMLAttributes = getRenderedAttributes(updatedNode, editor.extensionManager.attributes.filter((attribute) => attribute.type === updatedNode.type.name));
...
//#endregion
export { useEditor as a, require_jsx_runtime as c, EditorContent as i, src_default$1 as n, require_react_dom as o, src_default$2 as r, require_with_selector as s, src_default as t };
