...
import { applyCourierUpdate, assertAdmin, calculateCodAmount, courierActorClient, fullAddress, getCourierConfig, getOrderForBooking, normalizePhone, steadfastRequest } from "./couriers.server-C4Il9s5w.js";
...
var pathaoStores = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(pathaoStores_createServerFn_handler, async ({ context }) => {
	...
	const { persistCourierStores } = await import("./couriers.server-C4Il9s5w.js");
...
var bookPathao = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { courierStoreName, assertCourierStore } = await import("./couriers.server-C4Il9s5w.js");
	const conf = await getCourierConfig(supabase, "pathao");
...
var autoSyncCourierStatuses = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).handler(autoSyncCourierStatuses_createServerFn_handler, async ({ context }) => {
	const { syncPendingShipments } = await import("./couriers.server-C4Il9s5w.js");
	const { supabaseAdmin } = await import("./client.server-Db4xL-l4.js");
	try {
