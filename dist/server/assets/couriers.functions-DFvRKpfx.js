...
import { applyCourierUpdate, assertAdmin, calculateCodAmount, courierActorClient, fullAddress, getCourierConfig, getOrderForBooking, normalizePhone, steadfastRequest } from "./couriers.server-lywWCfFb.js";
...
var pathaoStores = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(pathaoStores_createServerFn_handler, async ({ context }) => {
	...
	const { persistCourierStores } = await import("./couriers.server-lywWCfFb.js");
...
var bookPathao = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { courierStoreName, assertCourierStore } = await import("./couriers.server-lywWCfFb.js");
	const conf = await getCourierConfig(supabase, "pathao");
