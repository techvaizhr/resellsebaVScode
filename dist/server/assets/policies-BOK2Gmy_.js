...
//#region src/lib/policies.ts
var DEFAULT_RESELLER_POLICIES = [
	{
		...
		sort_order: 1,
		is_active: true,
		updated_at: "2026-09-01T08:09:41.232Z"
	},
	{
		...
		sort_order: 2,
		is_active: true,
		updated_at: "2026-09-01T08:17:23.325Z"
	},
	{
		...
		sort_order: 3,
		is_active: true,
		updated_at: "2026-09-01T08:18:02.516Z"
	},
	{
		...
		sort_order: 4,
		is_active: true,
		updated_at: "2026-09-01T07:34:19.713Z"
	},
	{
	...
	},
	{
		...
		sort_order: 6,
		is_active: true,
		updated_at: "2026-09-01T08:20:46.813Z"
	},
	{
		...
		sort_order: 7,
		is_active: true,
		updated_at: "2026-09-01T08:21:26.264Z"
	},
	{
		...
		sort_order: 8,
		is_active: true,
		updated_at: "2026-09-01T08:21:52.986Z"
	},
	{
		...
		sort_order: 9,
		is_active: true,
		updated_at: "2026-09-01T08:22:29.597Z"
	},
	{
		...
		sort_order: 10,
		is_active: true,
		updated_at: "2026-09-01T08:22:48.804Z"
	}
];
...
function normalize(row) {
	return {
		...
		is_active: row.is_active !== false && row.is_active !== 0 && row.is_active !== "0" && row.is_active !== "false",
...
async function fetchActivePolicies() {
	try {
		const { data, error } = await supabase.from("reseller_policies").select("*").eq("is_active", true).order("sort_order", { ascending: true }).order("created_at", { ascending: true });
		if (error) throw error;
		if (data && data.length > 0) return data.map(normalize);
		return DEFAULT_RESELLER_POLICIES.filter((p) => p.is_active);
	} catch (err) {
		console.warn("fetchActivePolicies fallback to defaults:", err);
		return DEFAULT_RESELLER_POLICIES.filter((p) => p.is_active);
	}
}
...
async function fetchAllPolicies() {
	try {
		const { data, error } = await supabase.from("reseller_policies").select("*").order("sort_order", { ascending: true }).order("created_at", { ascending: true });
		if (error) throw error;
		if (data && data.length > 0) return data.map(normalize);
		return DEFAULT_RESELLER_POLICIES;
	} catch (err) {
		console.warn("fetchAllPolicies fallback to defaults:", err);
		return DEFAULT_RESELLER_POLICIES;
	}
}
...
//#endregion
export { pointsToText as i, fetchAllPolicies as n, pointsFromText as r, fetchActivePolicies as t };
