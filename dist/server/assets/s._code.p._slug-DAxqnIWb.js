import { a as deliveryLabel } from "./delivery-DY_nRbFK.js";
import { t as Route } from "./s._code.p._slug-P3ZO8-1x.js";
import { t as ProductCodeChip } from "./product-code-DC0CHuVs.js";
import { a as PrimaryButton, c as borderc, d as useStore, g as addToCart, h as trackViewContent, i as Price, l as cx, n as GhostButton, o as ProductGrid, p as trackAddToCart, r as Heading, s as SectionHead, t as EmptyState, u as muted } from "./ui-6GyuzhLW.js";
import { i as useResellerTools, n as ImageDownloadTools, r as stripHtml, t as CopyButton } from "./reseller-tools-8ryd3dba.js";
import { useEffect, useState } from "react";
import { Link, useNavigate } from "@tanstack/react-router";
import { jsx, jsxs } from "react/jsx-runtime";
import { toast } from "sonner";
import { ChevronLeft, Minus, Plus, ShieldCheck, ShoppingBag, Truck, Zap } from "lucide-react";
//#region src/routes/s.$code.p.$slug.tsx?tsr-split=component
function ProductPage() {
	const { code, slug } = Route.useParams();
	const nav = useNavigate();
	const store = useStore();
	const listing = store.bySlug(slug);
	const [qty, setQty] = useState(1);
	const [idx, setIdx] = useState(0);
	const tools = useResellerTools();
	useEffect(() => {
		setQty(1);
		setIdx(0);
		if (listing) trackViewContent({
			id: listing.product.id,
			name: store.title(listing),
			price: Number(listing.selling_price)
		});
	}, [listing?.id]);
	if (!listing) return /* @__PURE__ */ jsxs("div", {
		className: "mx-auto max-w-4xl px-4 py-16",
		children: [/* @__PURE__ */ jsx(EmptyState, {
			title: "Product not available",
			hint: "It may have been removed from this store."
		}), /* @__PURE__ */ jsx("div", {
			className: "mt-6 text-center",
			children: /* @__PURE__ */ jsx(Link, {
				to: "/s/$code",
				params: { code },
				children: /* @__PURE__ */ jsx(GhostButton, { children: "Back to store" })
			})
		})]
	});
	const p = listing.product;
	const title = store.title(listing);
	const price = Number(listing.selling_price);
	const images = p.product_images ?? [];
	const active = images[idx]?.url ?? store.image(listing);
	const inStock = p.stock === null || Number(p.stock) > 0;
	const imageUrls = images.map((im) => im.url).filter(Boolean);
	const detailsText = stripHtml([listing.custom_description || p.short_description || "", p.description || ""].filter(Boolean).join("\n\n"));
	const related = store.listings.filter((l) => l.id !== listing.id && l.product?.category_id === p.category_id).slice(0, 4);
	const jsonLd = {
		"@context": "https://schema.org/",
		"@type": "Product",
		name: title,
		description: listing.custom_description || p.short_description || title,
		image: active ? [active] : void 0,
		offers: {
			"@type": "Offer",
			priceCurrency: "BDT",
			price,
			availability: inStock ? "https://schema.org/InStock" : "https://schema.org/OutOfStock"
		}
	};
	function add(goCheckout) {
		addToCart(code, listing.id, qty);
		trackAddToCart({
			id: p.id,
			name: title,
			price,
			qty
		});
		if (goCheckout) nav({
			to: "/s/$code/checkout",
			params: { code }
		});
		else toast.success("Added to cart");
	}
	return /* @__PURE__ */ jsxs("div", {
		className: "mx-auto max-w-6xl px-4 pb-24 pt-8 lg:pb-10",
		children: [
			/* @__PURE__ */ jsx("script", {
				type: "application/ld+json",
				dangerouslySetInnerHTML: { __html: JSON.stringify(jsonLd) }
			}),
			/* @__PURE__ */ jsxs("nav", {
				className: cx("mb-5 flex items-center gap-1 text-xs", muted),
				children: [
					/* @__PURE__ */ jsxs(Link, {
						to: "/s/$code",
						params: { code },
						className: "inline-flex items-center gap-1 hover:text-[var(--st-primary)]",
						children: [/* @__PURE__ */ jsx(ChevronLeft, { className: "h-3 w-3" }), " Store"]
					}),
					/* @__PURE__ */ jsx("span", { children: "/" }),
					/* @__PURE__ */ jsx("span", {
						className: "truncate text-[var(--st-fg)]",
						children: title
					})
				]
			}),
			/* @__PURE__ */ jsxs("div", {
				className: "grid gap-10 lg:grid-cols-2",
				children: [/* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsxs("div", {
					className: cx("relative aspect-square overflow-hidden rounded-[var(--st-radius)] border bg-[var(--st-bg-alt)]", borderc),
					children: [active ? /* @__PURE__ */ jsx("img", {
						src: active,
						alt: title,
						className: "h-full w-full object-cover"
					}) : /* @__PURE__ */ jsx("div", {
						className: cx("grid h-full w-full place-items-center text-xs", muted),
						children: "No image"
					}), tools && /* @__PURE__ */ jsxs("div", {
						className: "absolute right-3 top-3 z-10 flex flex-col gap-2",
						children: [
							/* @__PURE__ */ jsx(ImageDownloadTools, {
								compact: true,
								images: imageUrls,
								activeUrl: active,
								baseName: title
							}),
							/* @__PURE__ */ jsx(CopyButton, {
								value: title,
								className: "h-9 w-9 rounded-full bg-[var(--st-surface)]/90 p-0 shadow-sm backdrop-blur"
							}),
							detailsText && /* @__PURE__ */ jsx(CopyButton, {
								value: detailsText,
								label: "details",
								className: "h-9 w-9 rounded-full bg-[var(--st-surface)]/90 p-0 shadow-sm backdrop-blur"
							})
						]
					})]
				}), images.length > 1 && /* @__PURE__ */ jsx("div", {
					className: "mt-3 flex flex-wrap gap-2",
					children: images.map((im, i) => /* @__PURE__ */ jsx("button", {
						onClick: () => setIdx(i),
						"aria-label": `Image ${i + 1}`,
						className: cx("h-16 w-16 flex-none overflow-hidden rounded-[var(--st-radius-sm)] border", i === idx ? "border-[var(--st-primary)]" : "border-[var(--st-border)]"),
						children: /* @__PURE__ */ jsx("img", {
							src: im.url,
							alt: "",
							loading: "lazy",
							className: "h-full w-full object-cover"
						})
					}, i))
				})] }), /* @__PURE__ */ jsxs("div", { children: [
					/* @__PURE__ */ jsxs("div", {
						className: "group flex items-start gap-2",
						children: [/* @__PURE__ */ jsx(Heading, {
							as: "h1",
							className: "text-2xl leading-tight md:text-4xl",
							children: title
						}), tools && /* @__PURE__ */ jsx(CopyButton, {
							value: title,
							className: "mt-1.5 flex-none opacity-70 group-hover:opacity-100"
						})]
					}),
					/* @__PURE__ */ jsx("div", {
						className: "mt-2",
						children: /* @__PURE__ */ jsx(ProductCodeChip, { code: p.product_code })
					}),
					(listing.custom_description || p.short_description) && /* @__PURE__ */ jsx("p", {
						className: cx("mt-3 text-sm leading-relaxed", "text-[var(--st-muted)]"),
						children: listing.custom_description || p.short_description
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "mt-5 flex items-baseline gap-3",
						children: [/* @__PURE__ */ jsx(Price, {
							value: price,
							className: "text-3xl md:text-4xl"
						}), /* @__PURE__ */ jsx("span", {
							className: cx("rounded-full px-2.5 py-1 text-[11px] font-semibold", inStock ? "bg-[var(--st-primary)]/12 text-[var(--st-primary)]" : "bg-[var(--st-bg-alt)] text-[var(--st-muted)]"),
							children: inStock ? "In stock" : "Out of stock"
						})]
					}),
					store.content.text("pdp_urgency") && inStock && /* @__PURE__ */ jsx("p", {
						className: "mt-3 text-sm font-medium text-[var(--st-primary)]",
						children: store.content.text("pdp_urgency")
					}),
					/* @__PURE__ */ jsxs("div", {
						className: cx("mt-5 grid gap-2 rounded-[var(--st-radius)] border p-4 text-sm", borderc),
						children: [/* @__PURE__ */ jsxs("div", {
							className: "flex items-center gap-2",
							children: [/* @__PURE__ */ jsx(Truck, { className: "h-4 w-4 text-[var(--st-primary)]" }), /* @__PURE__ */ jsxs("span", { children: ["Delivery: ", deliveryLabel(p)] })]
						}), [
							1,
							2,
							3
						].map((i) => store.content.text(`pdp_trust${i}`)).filter(Boolean).map((line) => /* @__PURE__ */ jsxs("div", {
							className: "flex items-center gap-2",
							children: [/* @__PURE__ */ jsx(ShieldCheck, { className: "h-4 w-4 text-[var(--st-primary)]" }), /* @__PURE__ */ jsx("span", { children: line })]
						}, line))]
					}),
					/* @__PURE__ */ jsxs("div", {
						className: "mt-6 flex flex-wrap items-center gap-3",
						children: [
							/* @__PURE__ */ jsxs("div", {
								className: cx("inline-flex items-center rounded-[var(--st-radius-sm)] border", borderc),
								children: [
									/* @__PURE__ */ jsx("button", {
										onClick: () => setQty((q) => Math.max(1, q - 1)),
										"aria-label": "Decrease",
										className: "p-3",
										children: /* @__PURE__ */ jsx(Minus, { className: "h-3.5 w-3.5" })
									}),
									/* @__PURE__ */ jsx("span", {
										className: "min-w-[3ch] text-center text-sm font-semibold",
										children: qty
									}),
									/* @__PURE__ */ jsx("button", {
										onClick: () => setQty((q) => q + 1),
										"aria-label": "Increase",
										className: "p-3",
										children: /* @__PURE__ */ jsx(Plus, { className: "h-3.5 w-3.5" })
									})
								]
							}),
							/* @__PURE__ */ jsxs(PrimaryButton, {
								disabled: !inStock,
								onClick: () => add(true),
								className: "flex-1",
								children: [/* @__PURE__ */ jsx(Zap, { className: "h-4 w-4" }), " Order now"]
							}),
							/* @__PURE__ */ jsxs(GhostButton, {
								disabled: !inStock,
								onClick: () => add(false),
								children: [/* @__PURE__ */ jsx(ShoppingBag, { className: "h-4 w-4" }), " Add to cart"]
							})
						]
					}),
					store.content.text("pdp_returns") && /* @__PURE__ */ jsx("p", {
						className: cx("mt-3 text-xs leading-relaxed", "text-[var(--st-muted)]"),
						children: store.content.text("pdp_returns")
					}),
					/* @__PURE__ */ jsx(Link, {
						to: "/login",
						search: { mode: "signup" },
						className: cx("mt-6 flex items-center justify-center gap-2 rounded-[var(--st-radius)] border px-4 py-3 text-sm font-medium", "bg-[var(--st-bg-alt)] hover:border-[var(--st-primary)] hover:text-[var(--st-primary)]", borderc),
						children: "Want to sell this product?"
					})
				] })]
			}),
			p.description && /* @__PURE__ */ jsxs("section", {
				className: "mt-12 lg:mt-16",
				children: [/* @__PURE__ */ jsxs("div", {
					className: "flex items-center justify-between gap-3",
					children: [/* @__PURE__ */ jsx(Heading, {
						className: "text-xl",
						children: "Product details"
					}), tools && /* @__PURE__ */ jsx(CopyButton, {
						value: detailsText,
						label: "details"
					})]
				}), /* @__PURE__ */ jsx("div", {
					className: cx("prose prose-sm mt-4 max-w-none rounded-[var(--st-radius)] border p-5 text-sm leading-relaxed", "border-[var(--st-border)]", "text-[var(--st-muted)]"),
					dangerouslySetInnerHTML: { __html: p.description }
				})]
			}),
			related.length > 0 && /* @__PURE__ */ jsxs("section", {
				className: "mt-16",
				children: [/* @__PURE__ */ jsx(SectionHead, {
					title: "You may also like",
					subtitle: "More from this collection"
				}), /* @__PURE__ */ jsx(ProductGrid, { listings: related })]
			}),
			store.content.flag("pdp_sticky") && /* @__PURE__ */ jsxs("div", {
				className: cx("fixed inset-x-0 bottom-0 z-40 flex items-center gap-3 border-t bg-[var(--st-surface)] px-4 py-3 lg:hidden", "border-[var(--st-border)]"),
				children: [/* @__PURE__ */ jsxs("div", {
					className: "min-w-0 flex-1",
					children: [/* @__PURE__ */ jsx("div", {
						className: "truncate text-xs text-[var(--st-muted)]",
						children: title
					}), /* @__PURE__ */ jsx(Price, {
						value: price,
						className: "text-lg"
					})]
				}), /* @__PURE__ */ jsxs(PrimaryButton, {
					disabled: !inStock,
					onClick: () => add(true),
					className: "px-4 py-2.5",
					children: [/* @__PURE__ */ jsx(Zap, { className: "h-4 w-4" }), " Order now"]
				})]
			})
		]
	});
}
//#endregion
export { ProductPage as component };
