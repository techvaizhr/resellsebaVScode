...
var getOrderDetails = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((data) => z.object({ orderId: z.string() }).parse(data)).handler(getOrderDetails_createServerFn_handler, async ({ data, context }) => {
	const { orderId } = data;
...
var recheckCourierStatus = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data) => z.object({ orderId: z.string() }).parse(data)).handler(recheckCourierStatus_createServerFn_handler, async ({ data, context }) => {
	const { supabase } = context;
	const { syncShipmentStatus } = await import("./couriers.server-g1OQ2MKy.js");
