...
var refreshDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(refreshDomain_createServerFn_handler, async ({ data, context }) => {
	const cf = await import("./cloudflare.server-DZyDw2hX.js");
	const db = context.supabase;
...
var setPrimaryDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(setPrimaryDomain_createServerFn_handler, async ({ data, context }) => {
	const db = context.supabase;
	const row = await loadDomainForCaller({
		supabase: context.supabase,
		userId: context.userId
...
var disconnectDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(disconnectDomain_createServerFn_handler, async ({ data, context }) => {
	const cf = await import("./cloudflare.server-DZyDw2hX.js");
	const db = context.supabase;
	const row = await loadDomainForCaller({
		supabase: context.supabase,
