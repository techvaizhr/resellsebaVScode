...
var saveCloudflareConfig = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	...
	server_note: z.string().max(2e3).default(""),
...
var listDomains = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
...
var connectDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
...
var refreshDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(createSsrRpc("2d5e71d3e2ba36243c3b1a733b0fe4b540d79894c95517bfe0f13a6928e404ab"));
/** Make one hostname the store's canonical domain. */
var setPrimaryDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(createSsrRpc("faaa580fd4cb98d1b12ecb9e0bb7a060ab3975a86f23abb9e6be865e25d51865"));
/** Remove the domain from Cloudflare (hostname + worker domain) and from the DB. */
var disconnectDomain = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ id: z.string().min(1) }).parse(d)).handler(createSsrRpc("d5dc6b97d303a369fb80b25d5071fcb7fc72bcb783d35eb1004728e1efbb00d5"));
...
var savePlatformOrigins = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
