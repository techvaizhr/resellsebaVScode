...
import { applyCourierUpdate, assertAdmin, calculateCodAmount, courierActorClient, fullAddress, getCourierConfig, getOrderForBooking, normalizePhone, steadfastRequest } from "./couriers.server-BlGlzEB3.js";
import { z } from "zod";
...
var bookSteadfast = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
...
var syncSteadfastStatus = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ shipmentId: z.string().uuid() }).parse(d)).handler(syncSteadfastStatus_createServerFn_handler, async ({ data, context }) => {
...
var steadfastCreateReturn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
...
var pathaoStores = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(pathaoStores_createServerFn_handler, async ({ context }) => {
	...
	const { persistCourierStores } = await import("./couriers.server-BlGlzEB3.js");
...
var pathaoPricePlan = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	cityId: z.number().int().positive(),
	zoneId: z.number().int().positive(),
	itemWeight: z.number().min(.5).max(10).optional(),
...
var bookPathao = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	orderId: z.string().uuid(),
	deliveryType: z.union([z.literal(48), z.literal(12)]).optional(),
	itemWeight: z.number().min(.5).max(10).optional(),
	...
	const { courierStoreName, assertCourierStore } = await import("./couriers.server-BlGlzEB3.js");
	const conf = await getCourierConfig(supabase, "pathao");
...
var syncPathaoStatus = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ shipmentId: z.string().uuid() }).parse(d)).handler(syncPathaoStatus_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
	await assertAdmin(supabase, userId);
...
var bookCarrybee = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	orderId: z.string().uuid(),
	...
	itemWeight: z.number().int().min(1).max(25e3).optional(),
	storeId: z.string().min(1).optional(),
...
var syncCarrybeeStatus = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ shipmentId: z.string().uuid() }).parse(d)).handler(syncCarrybeeStatus_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
	await assertAdmin(supabase, userId);
	const { carrybeeRequest } = await import("./carrybee.server-Cru9cHfp.js");
	const conf = await getCourierConfig(supabase, "carrybee");
...
var cancelCarrybee = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	shipmentId: z.string().uuid(),
...
var carrybeeReversePickup = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	shipmentId: z.string().uuid(),
...
var carrybeeExchange = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	shipmentId: z.string().uuid(),
	collectableAmount: z.number().int().min(0).max(1e5).optional(),
	itemWeight: z.number().int().min(1).max(25e3).optional(),
...
var receiveReturn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	orderId: z.string().uuid(),
	note: z.string().max(250).optional()
