...
import { applyCourierUpdate, assertAdmin, calculateCodAmount, courierActorClient, fullAddress, getCourierConfig, getOrderForBooking, normalizePhone, steadfastRequest } from "./couriers.server-IyT3q7Er.js";
...
var pathaoStores = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(pathaoStores_createServerFn_handler, async ({ context }) => {
	...
	const { persistCourierStores } = await import("./couriers.server-IyT3q7Er.js");
...
var bookPathao = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => objectType({
	...
	const { courierStoreName, assertCourierStore } = await import("./couriers.server-IyT3q7Er.js");
	const conf = await getCourierConfig(supabase, "pathao");
